...
export const useStrapiSeo = (seoData?: Seo) => {
  const setSeo = (data?: Seo) => {
    ...
    if (data.metaViewport) {
      useHead({
        meta: [{ name: 'viewport', content: data.metaViewport }],
      });
    }
    ...
    if (data.structuredData) {
      const structuredDataArray = Array.isArray(data.structuredData) ? data.structuredData : [data.structuredData];
            ...
            script: [
              {
                ...
                innerHTML: typeof item === 'string' ? item : JSON.stringify(item),
              },
            ],
          });
        }

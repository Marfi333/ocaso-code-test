<template>
  <div class="text-box">
    ...
    <div v-if="ctas && ctas.length > 0" class="ctas">
      <a v-for="(cta, index) in ctas" :key="index" :href="cta.target" :class="['cta', `cta--${cta.type}`]">
        <SharedStrapiImage v-if="cta.icon" :media="cta.icon" :alt="cta.icon.alternativeText || ''" class="cta__icon" />
        {{ cta.text }}
      </a>
    </div>
  </div>
